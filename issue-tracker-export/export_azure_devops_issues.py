...
AUTH_HEADER = b64encode(
    f":{os.environ.get('AZURE_DEVOPS_PAT', '')}".encode("utf8")
).decode("utf8")
...
def fetchWorkItemIds(
    baseURL: str, org: str, project: str, wiql: str, pageSize: int
) -> list[int]:
...
def fetchWorkItemDetails(
    baseURL: str, org: str, project: str, ids: list[int]
) -> Iterator[dict]:
...
def fetchWorkItems(
    baseURL: str, org: str, projects: list[str], start: str
) -> Iterator[dict]:
    pageSize = 200
...
def parsePullRequest(baseURL: str, org: str, project: str, pr: dict) -> PullRequest:
    ...
    return PullRequest(
        ...
        assignees=[],
        reviewers=[
            r["displayName"] for r in pr.get("reviewers", []) if r.get("displayName")
        ],
    )


def exportAzureDevOpsData(
    baseURL: str, org: str, projects: list[str], start: str, epicType: str
) -> IssueTrackerData:
    rawWorkItems = list(fetchWorkItems(baseURL, org, projects, start))
    workItems = [
        parsed
        for parsed in (parseWorkItem(baseURL, org, wi, epicType) for wi in rawWorkItems)
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Exports Azure DevOps work items into a format that can be analyzed by Sigrid."
    )
    parser.add_argument(
        "--ado-api-url",
        type=str,
    ...
    parser.add_argument(
        "--org", type=str, required=True, help="Azure DevOps organization name."
    )
    parser.add_argument(
        "--project",
        type=str,
    ...
    parser.add_argument(
        "--out",
        type=str,
        default=".sigrid/azure-devops-issues.json",
        help="Output file.",
    )
    parser.add_argument(
        "--start",
    ...
    data = exportAzureDevOpsData(
        args.ado_api_url, args.org, projects, args.start, args.epic_type
    )
    filterIssueData(data, excludeLabels)
    outputFile = os.path.expanduser(args.out)
    serialize(data, outputFile, args.anonymize)
    print(
        f"Exported {len(data.workItems)} work items and {len(data.pullRequests)} pull requests to {outputFile}"
    )
