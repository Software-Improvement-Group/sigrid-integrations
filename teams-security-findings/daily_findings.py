...
class TeamsAPI:
    def __init__(self, webhook_uri: str):
        self.teams_webhook_uri = webhook_uri
    ...
    def post_message(self, message: str) -> bool:
        try:
            ...
            body = str.encode(json.dumps(card))
            req = request.Request(self.teams_webhook_uri, body, method='POST')
            ...
            LOG.error('protocol error trying to post to %s', self.teams_webhook_uri)
            return False
    ...
    def handle_response(response):
        if response.status == 200:
            ...
            LOG.error('Calling incoming Teams webhook failed: status=%s', response.status)
            return False
...
def create_message(system: str, findings: list[Finding], num_findings: int) -> Union[str, None]:
    if len(findings) == 0:
        ...
        message = f'{len(findings)} new open findings in {system} during the last week'
    if len(findings) > 5:
        message += '. The 5 most severe ones are:\n'
    else:
        message += '.\n'
    for finding in findings[:5]:
        message += f'[{finding.severity}, {finding.status}] {finding.type} '
...
if __name__ == "__main__":
    parser = ArgumentParser(description='Gets open security findings and post them to Microsoft Teams.')
    ...
    teams_webhook_uri = os.getenv('TEAMS_WEBHOOK')
    if not TeamsAPI.is_valid_webhook(teams_webhook_uri):
        print('Missing or incomplete environment variable TEAMS_WEBHOOK')
        sys.exit(1)

    logging.basicConfig(encoding='utf-8', level=logging.INFO)

    teams = TeamsAPI(teams_webhook_uri)
