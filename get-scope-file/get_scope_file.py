    ...
    try:
        with urllib.request.urlopen(request) as response:
            if response.status == 204:
                print(
                    f"Sigrid cannot find metadata for this system (HTTP status {response.status})"
                )
                sys.exit(1)
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Retrieves and dumps the Sigrid scope configuration file for a system."
    )
    ...
    parser.add_argument(
        "--sigridurl", type=str, default="https://sigrid-says.com", help=SUPPRESS
    )
    ...
    try:
        with urllib.request.urlopen(request) as response:
            if response.status == 204:
                print(
                    f"Sigrid cannot find analysis results for this system (HTTP status {response.status})"
                )
                sys.exit(1)
