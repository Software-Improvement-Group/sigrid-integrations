...
class SlackAPI:
    ...
    def handle_response(response):
        if response.status == 200:
            ...
            LOG.error(
                "Calling incoming Slack webhook failed: status=%s", response.status
            )
            return False
...
def process_findings(
    ...
    if len(result) == 0:
        ...
        return sorted(
            result,
            key=lambda x: (x.severity_score, x.first_seen_snapshot_date),
            reverse=True,
        )
...
def create_message(
    system: str, findings: list[Finding], num_findings: int
) -> Union[str, None]:
    if len(findings) == 0:
        ...
        message = (
            f"{len(findings)} new open findings in _{system}_ during the last week"
        )
    if len(findings) > 5:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=True,
        help="Controls posting of celebratory message when there are no new open findings.",
    )
