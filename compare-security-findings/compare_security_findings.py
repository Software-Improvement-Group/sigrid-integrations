...
def fetch_api_data(customer: str, token: str, system: str) -> Any:
    ...
    try:
        ...
        if e.code == 403:
            ...
            logger.error(
                "Resource not found. Please check the customer name and system namesprovided."
            )
            raise RuntimeError(
                "Resource not found. Please check the customer name and system names provided."
            ) from e
        else:
...
def find_fuzzy_match(
    new_finding: Dict, candidate_findings: List[Dict], line_tolerance: int = 10
) -> Optional[Dict]:
...
def compare_findings(
    main_findings: List[Dict], new_findings: List[Dict], line_tolerance: int
) -> List[Dict]:
    ...
    for finding in new_findings:
        ...
        if exact_key not in main_exact_keys:
            ...
            fuzzy_match = find_fuzzy_match(
                finding, candidate_main_findings, line_tolerance
            )
    ...
    logger.debug(
        f"Found {len(new_only_findings)} new findings not present in main system"
    )
...
def group_findings_by_status(findings: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    status_groups = {"FIXED": [], "FALSE_POSITIVE_RISK_ACCEPTED": [], "RAW_REFINED": []}
    ...
    logger.debug(
        f"Grouped findings - Fixed: {len(status_groups['FIXED'])}, "
...
def flatten_finding(finding: Dict) -> Dict:
    ...
    if "_fuzzy_match" in finding:
        ...
        flattened["Fuzzy Match Lines"] = (
            f"{fuzzy.get('startLine', '')}-{fuzzy.get('endLine', '')}"
        )
...
def create_status_sheet(
    writer: pd.ExcelWriter, findings: List[Dict], sheet_name: str
) -> bool:
    if not findings:
        return False
...
def create_excel_report(grouped_findings: Dict[str, List[Dict]], output_file: str):
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        sheets_created += create_status_sheet(
            writer, grouped_findings["FIXED"], "Fixed"
        )
        sheets_created += create_status_sheet(
            writer,
            grouped_findings["FALSE_POSITIVE_RISK_ACCEPTED"],
            "False Positive-Risk Accepted",
        )
        sheets_created += create_status_sheet(
            writer, grouped_findings["RAW_REFINED"], "Raw-Refined"
        )
        ...
        if sheets_created == 0:
            ...
            logger.debug(
                f"Excel file created successfully with {sheets_created} sheet(s): {output_file}"
            )


def process_api_output(
    main_system_data: str, new_system_data: str, output_file: str, line_tolerance: int
):
    try:
        ...
        new_only_findings = compare_findings(
            main_findings, new_findings, line_tolerance
        )
...
def validate_output_filename(value):
    ...
    if "." in value:
        raise argparse.ArgumentTypeError(
            f"The output file has an incorrect extension. Only .xlsx is supported. You provided: {value}"
        )
    return value + ".xlsx"
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Compare security findings between two systems in Sigrid"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--token",
        type=str,
        help="Sigrid API token. If not provided, the SIGRID_CI_TOKEN environment variable will be used.",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The main branch of the system, the name has to be exactly the same as 'System name' in Sigrid.",
    )
    parser.add_argument(
        "--new-system",
        type=str,
        required=True,
        help="The new system/branch that is about to be merged into main.",
    )
    parser.add_argument(
        "--line-tolerance",
        type=int,
        default=50,
        help="Line number tolerance for fuzzy matching findings (default: 50).",
    )
...
def main():
    ...
    if not token:
        logger.error(
            "Missing Sigrid API token in arguments and in environment variable SIGRID_CI_TOKEN"
        )
        sys.exit(1)
    ...
    if args.output:
        ...
        output_file = (
            f"{customer_name}-{main_system}-{new_system}-security-findings.xlsx"
        )
    ...
    try:
        ...
        process_api_output(
            main_system_data, new_system_data, output_file, line_tolerance
        )
