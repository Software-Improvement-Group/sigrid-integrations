...
class SigridApiClient:
    ...
    def fetchLastUpdated(self):
        response = self.callEndPoint(f"/maintainability/{self.customer}")
        return sorted(response["systems"], key=lambda e: -e["maintainabilityDate"])

    def fetchArchitectureRaw(self, system):
        response = self.callEndPoint(f"/architecture-quality/{self.customer}/{system}/raw")
        return response
