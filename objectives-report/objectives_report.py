...
def generateGroupedObjectivesStatusChart(calculator, outputFile):
    ...
    statusPercentages = {
        type: calculator.calculateStatus(systems, type) for type in types
    }
    ...
    chart = HorizontalStackedBar(
        ...
        height=400,
        range=(0, 100),
        style=getChartStyle(STATUS_COLORS),
    ...
    chart.add(
        "Incomplete", [statusPercentages[type][Status.INCOMPLETE] for type in types]
    )
...
def generateTeamObjectivesStatusChart(calculator, outputFile):
    ...
    statusPercentages = {
        team: calculator.calculateStatus(systems)
        for team, systems in teamSystems.items()
    }
    ...
    chart.value_formatter = lambda value: f"{round(value)}%"
    chart.add(
        "Complete", [statusPercentages[team][Status.COMPLETE] for team in teamSystems]
    )
    chart.add(
        "Incomplete",
        [statusPercentages[team][Status.INCOMPLETE] for team in teamSystems],
    )
    chart.add(
        "Not covered", [statusPercentages[team][Status.NA] for team in teamSystems]
    )
    chart.render_to_file(outputFile)
...
def generateGroupedObjectivesTrendChart(calculator, type, systemFilter, outputFile):
    ...
    trend = {
        period: calculator.calculateStatus(
            systemFilter.apply(calculator.trend[period], metadata), type
        )
...
def generateOverallObjectivesBarChart(calculator, systemFilter, outputFile):
    ...
    trend = {
        period: calculator.calculateStatus(
            systemFilter.apply(calculator.trend[period], metadata)
        )
        for period in periods
    }
...
def generateOverallObjectivesLineChart(calculator, systemFilter, outputFile):
    ...
    chart = Line(
        ...
        height=400,
        range=(0, 100),
        style=getChartStyle(TEAM_COLORS),
        legend_at_bottom=True,
    )
    chart.title = "Sigrid objective trend"
    chart.x_labels = [period.start.strftime("%m/%Y") for period in periods]
    ...
    for type in calculator.getAvailableObjectiveTypes(calculator.status):
        ...
        chart.add(
            ...
            [trend[period][Status.COMPLETE] for period in periods],
        )
    chart.render_to_file(outputFile)
...
def generateMetadataCompletionChart(calculator, group, outputFile):
    ...
    withoutMetadata = [
        len(systems) - countMetadataComplete(systems)
    ...
    chart = HorizontalStackedBar(
        width=600,
        height=400,
        style=getChartStyle(METADATA_COLORS),
        legend_at_bottom=True,
    )
    ...
    for name in sorted(groupedSystems.keys()):
        ...
        status = [
            calculator.calculateStatus(
                calculator.trend[period], type, filter=systemFilter
            )[Status.COMPLETE]
            for period in periods
        ]
...
def formatTitle(objective):
    ...
    conditions = [
        f"{formatField(field)}: {formatValues(values)}"
        for field, values in objective["conditions"].items()
    ]
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Generates a progress report based on a your Sigrid portfolio objectives."
    )
    parser.add_argument("customer", type=str, help="Sigrid customer name.")
    parser.add_argument(
        "--start",
        type=str,
        default=LAST_YEAR.strftime("%Y-%m-%d"),
    ...
    parser.add_argument(
        "--out", type=str, default=os.getcwd(), help="Output directory path."
    )
    ...
    sigrid = SigridApiClient(
        args.sigridurl, args.customer, os.environ["SIGRID_CI_TOKEN"]
    )
    ...
    generateOverallObjectivesBarChart(
        calculator, SystemFilter(), f"{outputDir}/all-trend-bar.svg"
    )
    generateOverallObjectivesLineChart(
        calculator, SystemFilter(), f"{outputDir}/all-trend-line.svg"
    )

    for type in calculator.getAvailableObjectiveTypes(calculator.status):
        generateGroupedObjectivesTrendChart(
            calculator, type, SystemFilter(), f"{outputDir}/trend-{type.lower()}.svg"
        )
        generateObjectivesBreakdownChart(
            calculator, type, divisions, f"{outputDir}/breakdown-divisions-{type}.svg"
        )
        generateObjectivesBreakdownChart(
            calculator, type, teams, f"{outputDir}/breakdown-teams-{type}.svg"
        )
    ...
    for division, systems in divisions.items():
        ...
        for type in calculator.getAvailableObjectiveTypes(systems):
            ...
            generateGroupedObjectivesTrendChart(
                ...
                type,
                systemFilter,
                f"{divisionDir}/trend-{type.lower()}.svg",
            )
    ...
    for team, systems in teams.items():
        ...
        for type in calculator.getAvailableObjectiveTypes(systems):
            ...
            generateGroupedObjectivesTrendChart(
                calculator, type, systemFilter, f"{teamDir}/trend-{type.lower()}.svg"
            )
