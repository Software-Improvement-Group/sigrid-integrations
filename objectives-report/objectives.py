...
class SystemFilter:
    ...
    def check(self, systemEvaluation, metadata):
        ...
        divisionMatch = self.division is None or self.division == (
            systemMetadata["divisionName"] or "Unknown"
        )
        teamMatch = self.team is None or self.team in (
            systemMetadata["teamNames"] or "Unknown"
        )
...
class ObjectivesCalculator:
    def __init__(self, sigrid, startDate, endDate):
        ...
        self.rawPortfolioObjectives = sorted(
            sigrid.fetchPortfolioObjectives(), key=lambda o: o["objective"]["type"]
        )
        self.status = sigrid.fetchObjectivesEvaluation(startDate, endDate)
        self.trend = {
            period: sigrid.fetchObjectivesEvaluation(period.start, period.end)
    ...
    def getAvailableObjectiveTypes(self, systemEvaluations):
        found = set(
            soe["type"] for system in systemEvaluations for soe in system["objectives"]
        )
    ...
    def calculateStatus(self, systemEvaluations, type="*", *, filter=None):
        ...
        return {
            status: (count * 100.0 / total if total > 0 else 0)
